(ns morningstar.main
  "Kotodama WASM entrypoint for the Morningstar clean-room actor (L4) — Clojure port.
  ...
  facts (`morningstar.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "morningstar")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "mornings_ins"
   ...
   {:entity "Execution"  :plural "executions"  :id-prefix "mornings_exe"
    :fields [:orderId :quantity :price :executedAt] :required [:quantity :price]
    :coerce {:quantity :float :price :float}   :refs {:orderId "Order"}}
   {:entity "MarketData" :plural "marketdatas" :id-prefix "mornings_mar"
   ...
   {:entity "Position"   :plural "positions"   :id-prefix "mornings_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"morningstar.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "morningstar-compat" :tier tier :entities entities} 200])
...
(defn start! [] :morningstar-compat/ready)
